...
<!-- ECZ-ID:PASSPORT-ACQUISITION:BEGIN generated by 90-scripts/generate_passport_distribution.py - do not edit inside this fence -->

## Free ECZ-ID Passports

If the thing you checked has no public ECZ-ID proof and **you operate it**, it can have a
persistent ECZ-ID identity for nothing:

| Target | Passport | Price | Minimum Parent |
|---|---|---|---|
| MCP server | **ECZ-ID MCP Passport** | **FREE**, non-sellable | DECLARED |
| Agent | **ECZ-ID Agent Passport** | **FREE**, non-sellable | DECLARED |

- **Get FREE ECZ-ID MCP Passport** / **Get FREE ECZ-ID Agent Passport** if you operate the target.
- **Request ECZ-ID Passport** if you do not. A request is not proof, not ownership and not
  authority, and it never discloses who asked.
- Acquisition creates or reuses the free DECLARED Parent in the same journey. One
  authentication. Nothing to pay.
- The handoff carries the Passport type (`requested_passport_type`), which the acquisition
  door accepts and preserves across sign-in. Nothing is retyped.

Whether the acquisition route is open right now is resolved from live backend state at the
point of need, by the surface you are using. This document does not assert it. When the
route is closed, that surface says so and **Request ECZ-ID Passport** remains available.

### What a free Passport is not

It gives a machine a persistent ECZ-ID identity. It does **not** mean verified, assured,
safe, secure, compliant, approved, certified or endorsed.

And the absence of one means nothing bad: No public ECZ-ID Resolver proof found. This does not mean the entity, agent, MCP or service is unsafe or untrustworthy.

You never need an account to *verify*. Authentication begins only if you choose to acquire.

ECZ-ID Core issues identity; TrustOps orchestrates acquisition. This tool initiates and
reports — it never issues identity itself. Re-check the public Resolver before relying on this result.

<!-- ECZ-ID:PASSPORT-ACQUISITION:END -->
...
jobs:
  verify:
    ...
    steps:
      - uses: Ecocitizenz/ecz-id-mcp-verifier@v0.9.1
        with:
...
Outputs: `result-state`, `reason-codes`, `action-envelope-json`, `setup-handoff-json`, `mcp-action-envelope-json`, `request-to-resolve-json`, `passport-opportunity-json`, `primary-action`, `trustops-action-url`, `developer-guidance-url`. The Action also writes a concise GitHub **step summary**.

<!-- ECZ-ID:PASSPORT-ACQUISITION-CI:BEGIN generated by 90-scripts/generate_passport_distribution.py - do not edit inside this fence -->

### Passport guidance in CI

A missing ECZ-ID Passport **never fails the build by default**. Absence is not a finding.

When the checked target is an MCP server or an agent and no public proof was found, the
run summary carries:

```
No public ECZ-ID Resolver proof found. This does not mean the entity, agent, MCP or service is unsafe or untrustworthy.

If you operate this target: Get FREE ECZ-ID MCP Passport / Get FREE ECZ-ID Agent Passport
                            - free, non-sellable, DECLARED Parent, one sign-in.
If you do not:              Request ECZ-ID Passport
Re-check the public Resolver before relying on this result.
```

The structured output `passport-opportunity-json` names the canonical type
(`MCP_PASSPORT` or `AGENT_PASSPORT`) and the handoff carries
`requested_passport_type`, so nothing is retyped downstream.

The Passport route appears in the summary and in the structured output. It is never
written into the result state, the reason codes or the exit code: conversion must not be
able to change what the check found.

<!-- ECZ-ID:PASSPORT-ACQUISITION-CI:END -->
...
- **GitHub Action:** `Ecocitizenz/ecz-id-mcp-verifier@v0.9.1` on the GitHub Actions Marketplace.
