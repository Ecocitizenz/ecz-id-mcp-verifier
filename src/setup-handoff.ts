...
// Posture describes how the verifier views the target's current state for
// routing purposes. This is not a safety claim.
export type Posture =
  | "VERIFIABLE"
  | "MISSING_PROOF"
  | "DEGRADED"
...
const DEVELOPER_PATH_BY_TYPE: Record<TargetType, string> = {
  mcp_server: "/mcp",
...
export function developerGuidanceUrlFor(
  targetType: TargetType,
  developerBase: string = DEVELOPER_GATEWAY
): string {
...
// ---------------------------------------------------------------------------
// TrustOps URL builder. Strict allow-list of query params.
// ---------------------------------------------------------------------------
 ...
 * TrustOps side.
 */
export const REQUESTED_PASSPORT_TYPES = ["agent", "mcp"] as const;
export type RequestedPassportType = (typeof REQUESTED_PASSPORT_TYPES)[number];

const PASSPORT_TYPE_BY_TARGET: Partial<Record<TargetType, RequestedPassportType>> = {
  mcp_server: "mcp",
 ...
 */
export function requestedPassportTypeFor(
  targetType: TargetType
): RequestedPassportType | undefined {
...
export function allowedTrustopsParams(): readonly string[] {
...
const RETURN_TO_BANNED_SUBSTRINGS: readonly string[] = [
  "secret",
  "token",
  "api_key",
  "apikey",
...
export function isAllowedReturnToUrl(value: unknown): boolean {
  ...
  if (!(ALLOWED_RETURN_TO_HOSTS as readonly string[]).includes(u.hostname)) {
    return false;
  }
...
export function sanitizeReturnToUrl(value: unknown): string | null {
  return isAllowedReturnToUrl(value) ? (value as string) : null;
}

export function buildTrustopsUrl(input: BuildTrustopsUrlInput): string {
...
const MISSING_PROOF_STATES: ReadonlySet<ResultState> = new Set([
...
const DEGRADED_STATES: ReadonlySet<ResultState> = new Set([
  "DEGRADED",
  "MISMATCH",
  "EXPIRED",
  "SUSPENDED",
...
const NOT_ROUTABLE_AS_ACTIVE_STATES: ReadonlySet<ResultState> = new Set([
  "UNSUPPORTED_TARGET",
...
function postureFor(state: ResultState): Posture {
  if (state === "RESOLVER_VERIFIABLE") return "VERIFIABLE";
  if (MISSING_PROOF_STATES.has(state)) return "MISSING_PROOF";
  if (DEGRADED_STATES.has(state)) return "DEGRADED";
  if (NOT_ROUTABLE_AS_ACTIVE_STATES.has(state)) return "NOT_ROUTABLE_AS_ACTIVE";
...
function planFor(state: ResultState, operator: OperatorMode): ActionPlan {
...
export function buildSetupHandoff(
  ...
  const developerUrl = developerGuidanceUrlFor(
    input.target_type,
    input.developer_base_url
  );
