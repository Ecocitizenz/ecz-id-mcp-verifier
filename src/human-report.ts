...
export function toHumanReport(result: VerifyResult): string {
  ...
  switch (result.result_state) {
    ...
    case "NO_PUBLIC_RESOLVER_PROOF_FOUND":
      ...
      lines.push(
        `If you do not operate it: share resolver guidance -> ${flow.developer_guidance_url}`
      );
    ...
    case "OBSERVED":
      ...
      lines.push(
        `  If you operate this target: start setup in TrustOps -> ${flow.trustops_action_url}`
      );
      lines.push(
        `  If you do not: share developer guidance with the operator -> ${flow.developer_guidance_url}`
      );
      break;
    ...
    case "DEGRADED":
      ...
      lines.push("Operator path:");
      lines.push(
        `  If you operate this target: start repair in TrustOps -> ${flow.trustops_action_url}`
      );
      lines.push(
        `  If you do not: re-check before reliance and share repair guidance -> ${flow.developer_guidance_url}`
      );
      break;
    case "REVOKED":
    case "SUSPENDED":
    case "EXPIRED":
      ...
      lines.push(
        `  If you do not: share repair guidance -> ${flow.developer_guidance_url}`
      );
      break;
    default:
  ...
  if (
    !isResolved(result.result_state) &&
    ...
    lines.push(
      `Open TrustOps setup if you operate this target: ${result.trustops_base_url}`
    );
