 ...
 */
export function readActionInput(name, env = process.env) {
 ...
 */
export function actionArgv(env = process.env) {
    const argv = [];
    ...
    if (targetType !== undefined)
        argv.push("--target-type", targetType);
    const policy = readActionInput("policy", env);
    if (policy !== undefined)
        argv.push("--policy", policy);
    ...
    const resolverBase = readActionInput("resolver-base", env) ?? readActionInput("resolver-url", env);
    if (resolverBase !== undefined)
        argv.push("--resolver-base", resolverBase);
    const timeoutMs = readActionInput("timeout-ms", env);
    if (timeoutMs !== undefined)
        argv.push("--timeout-ms", timeoutMs);
    // Network opt-out: either `offline` or `no-network`, truthy -> offline.
    if (isTruthy(readActionInput("offline", env)) ||
    ...
    if (isTruthy(readActionInput("report", env)))
        argv.push("--report");
    if (isTruthy(readActionInput("json", env)))
        argv.push("--json");
    if (isTruthy(readActionInput("actions", env)))
        argv.push("--actions");
    return argv;
}
