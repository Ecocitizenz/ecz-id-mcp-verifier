...
import { buildEnvelope } from "./action-envelope.js";
import { buildMcpActionEnvelope, buildRequestToResolve } from "./result-actions.js";
import { toHumanReport } from "./human-report.js";
import { buildCapabilities, buildMcpConfig, runDoctor, toDoctorHuman } from "./capabilities.js";
import { computeExitCode, EXIT_INTERNAL, EXIT_UNSUPPORTED_OR_INVALID } from "./exit-codes.js";
import { POLICY_MODES } from "./policy.js";
import { OPERATOR_MODES } from "./setup-handoff.js";
import { RESOLVER_BASE, TRUSTOPS_START, DEVELOPER_GATEWAY, VERIFIER_NAME, VERIFIER_VERSION, PACKAGE_NAME } from "./constants.js";
...
function strFlag(flags, k) {
    const v = flags[k];
    return typeof v === "string" ? v : undefined;
}
function boolFlag(flags, k) {
    const v = flags[k];
    if (v === true)
        return true;
    if (typeof v === "string") {
...
export async function runCli(argv) {
    ...
    if (!POLICY_MODES.includes(policyRaw)) {
        return {
            exit_code: EXIT_UNSUPPORTED_OR_INVALID,
            stdout: "",
    ...
    if (!OPERATOR_MODES.includes(operatorRaw)) {
        return {
            exit_code: EXIT_UNSUPPORTED_OR_INVALID,
            stdout: "",
    ...
    const noNetwork = boolFlag(flags, "offline") || boolFlag(flags, "no-network");
    ...
    try {
        ...
        const networkAttemptedAndFailed = result.network_attempted && Boolean(result.network_error);
        ...
        if (wantReport && !wantJson) {
            ...
            primaryOutput =
                toHumanReport(result) +
                    "\n\n" +
                    toJson(buildJsonOutput(result, {
                        exit_code,
                        action_envelope: wantActions ? envelope : null
                    }));
        }
        else {
            primaryOutput = toJson(buildJsonOutput(result, {
                exit_code,
                action_envelope: wantActions ? envelope : null
            }));
        }
        ...
        if (policy === "PREFER" &&
            ...
            stderrParts.push("Warning: no public resolver proof found yet. Local policy decides.");
        }
        const mcpActionEnvelope = buildMcpActionEnvelope(result);
        const requestToResolve = buildRequestToResolve(result);
        const gh_outputs = `result-state=${result.result_state}\n` +
