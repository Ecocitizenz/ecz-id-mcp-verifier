 ...
 */
const MISSING_PASSPORT_STATES = new Set([
...
export function buildPassportOpportunity(input) {
    ...
    if (passportType === undefined)
        return null;
    if (!MISSING_PASSPORT_STATES.has(input.result_state))
        return null;
    ...
    return {
        ...
        acquisition_availability: {
            ...
            note: "This surface is local-first and does not probe the issuing backend. The Passport " +
 ...
 */
export function renderPassportOpportunityLine(opportunity) {
    if (opportunity === null)
        return null;
    ...
    if (claim !== undefined)
        return `${claim.label} - free of charge, and no account is needed to verify.`;
