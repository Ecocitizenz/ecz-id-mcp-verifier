...
import { buildJsonOutput, toJson } from "../output.js";
...
export async function runRecheckResolver(args) {
    const result = await verify({
        target: args.target,
        policy: "OPEN",
        noNetwork: args.offline === true
...
const STATE_EXPLANATIONS = {
    RESOLVER_VERIFIABLE: "Public Resolver proof was found for this target. Informational only; re-check before reliance and let local policy decide.",
    NO_PUBLIC_RESOLVER_PROOF_FOUND: "No public Resolver proof was found yet. This does not indicate a problem with the target; it only means no public proof is published. Local policy decides.",
    PARTIAL_PUBLIC_PROOF_FOUND: "Some, but not all, expected public proof was found. Informational only; local policy decides.",
    SETUP_REQUIRED: "Public proof would require the operator to complete setup. Routing-only; the verifier never completes setup.",
    ...
    DEGRADED: "The public proof projection is degraded (for example stale). Treated as missing proof for re-check purposes; never as positive proof.",
    MISMATCH: "Observed public attributes did not match the expected proof. Informational only; not a safety verdict.",
    ...
    UNSUPPORTED_TARGET: "This target shape is not supported for public Resolver proof. No network lookup is performed.",
    LEGACY_ALIAS_NOT_ACTIVE_SKU: "The target resolves to a legacy alias that is not an active product. Routing-only.",
    REJECTED_PRODUCT_NOT_SELLABLE: "The target maps to a product that is not currently sellable. Routing-only.",
    DEFERRED_PRODUCT_NOT_SELLABLE: "The target maps to a product that is deferred and not currently sellable. Routing-only.",
...
const REASON_EXPLANATIONS = {
    NO_PUBLIC_RESOLVER_PROOF_FOUND: "No public Resolver proof was found yet. Not a problem signal; local policy decides.",
    ...
    RESOLVER_RESPONSE_UNVERIFIABLE: "A Resolver response could not be safely interpreted as valid proof; treated as missing proof, never as positive proof.",
...
const NEUTRAL_UNKNOWN = "Unrecognised code; no canonical explanation. Not a decision and not a safety verdict.";
export function runExplainResult(args) {
    const codes = Array.isArray(args.reason_codes) ? args.reason_codes : [];
    const reason_codes = codes.map((c) => {
        const recognized = REASON_CODES.includes(c);
        return {
            code: c,
            recognized,
            explanation: recognized ? REASON_EXPLANATIONS[c] : NEUTRAL_UNKNOWN
        };
    });
    let stateBlock = null;
    if (typeof args.result_state === "string" && args.result_state.length > 0) {
        const recognized = RESULT_STATES.includes(args.result_state);
        stateBlock = {
            state: args.result_state,
            recognized,
            explanation: recognized ? STATE_EXPLANATIONS[args.result_state] : NEUTRAL_UNKNOWN
        };
    }
    return {
