 ...
 */
const privacyFields = Object.fromEntries(Object.keys(OUTPUT_PRIVACY_FIELDS).map((k) => [k, z.literal(true)]));
...
export const checkTargetOutputSchema = z
    ...
    .describe("The canonical ECZ-ID verifier result contract. Informational and routing-only: never a safety, approval or compliance verdict.");
export const recheckResolverOutputSchema = z
    ...
    .describe("A read-only public Resolver re-check projection. Never positive proof on its own; re-check before reliance.");
/** The three framing invariants `ecz_explain_result` also carries. */
const explainInvariants = Object.fromEntries(["no_safety_or_approval_inference", "local_policy_decides", "recheck_before_reliance"].map((k) => [k, z.literal(true)]));
export const explainResultOutputSchema = z
    ...
    result_state: z
        ...
        .nullable(),
    reason_codes: z.array(z.looseObject({
        code: z.string(),
        recognized: z.boolean(),
        explanation: z.string()
    })),
    ...
    .describe("Public-safe explanations of EXISTING canonical states and reason codes. Invents no new codes and emits no allow/deny decision.");
